...
import { View, Text, Alert, StyleSheet,FlatList,
  ScrollView,Pressable ,TouchableOpacity} from 'react-native';
  import { internationalServices } from '../services/InternationalService';
import {
  NativeBaseProvider,
  Divider,
  Image,
...
docRef.get().then((doc) => {
    if (doc.exists) {
        console.log("Document data:", doc.data().favInternational);
        if(doc.data().favInternational===false){
          setFavourite('heart')
        }
        else if(doc.data().favInternational===true){
          setFavourite('hearto')
        }  
    } else {
